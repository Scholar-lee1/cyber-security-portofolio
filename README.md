# Cyber Security Portfolio

> Building elite-level competency. Not certification chasing—real skills.

## What This Is

A professional portfolio documenting progression through cyber security fundamentals. Each activities goes beyond completion to demonstrate **understanding**: the why behind the what.

**Philosophy:** Think like an attacker. Build like a defender. Document like a professional.

---
## Activities

### 1. Linux Fundamentals
**Status:** In Progress



[→ View Activity 1](/1-linux-fundamentals/)

### 2. OverTheWire Bandit
**Status:** Coming Soon

Solving Bandit Levels 0-34. Real hacking challenges that teach Linux problem-solving through practice.

[→ View Activity 2](/2-bandit/)

### 3. Network Security Analysis
**Status:** Coming Soon

Capturing and analyzing network traffic. Understanding protocols, identifying threats, building defensive intuition.

[→ View Activity 3](/3-network-security/)

### 4. DVWA Walkthrough (Compulsory)
**Status:** Available

Deliberately Vulnerable Web Application—intentionally breaking into web apps, documenting vulnerabilities, explaining defenses.

[→ View Activity 4](/Damn-Vulnerable-WebApplication/)

---
## The Approach

Each activities follows this structure:
- **Execute:** Do the work (run commands, solve challenges, exploit vulnerabilities)
- **Understand:** Explain why it works
- **Document:** Screenshot + context + learning
- **Professional:** Clean repo, clear structure, portfolio-ready

---
## Philosophy

> Competency beats certification.
> 
> Real security professionals understand the systems they defend. They can attack intelligently. They can think.
> 
> This portfolio is built to show that kind of thinking.

Aspiring to high-level standards: elite-level fundamentals, practical application, clear documentation.

---
## Structure

---
## Started
September 2026

**Goal:** Elite-level fundamentals by September 2027.

---
Made with intention. Built for mastery.
