# DVWA Walkthrough — Web Application Security Fundamentals

> Deliberately Vulnerable Web Application: Learning security by breaking it intentionally.

---
## Quick Start

### Installation & Setup

**For complete installation guide, see:**
[DVWA Installation & Configuration](https://github.com/Los-merengue/Walkthrough/blob/main/dvwa/install-and-configure-dvwa.md)
(Credit: Emeka Michael Nzeopara's detailed walkthrough)

**Screenshots included below:**

### 1. Download DVWA
<img width="963" height="856" alt="Screenshot 2026-09-19 025000" src="https://github.com/user-attachments/assets/80ddb69a-463f-4c12-b922-3517b86fe52f" />

---
### 2. DVWA Web Interface
<img width="997" height="848" alt="Screenshot 2026-09-21 014143" src="https://github.com/user-attachments/assets/0160e773-341a-4398-a9fe-9aebe877c1b9" />

Default credentials:
- Username: `admin`
- Password: `password`

---
### 3. How to Change Difficulty Levels

<img width="957" height="864" alt="Screenshot 2026-09-19 025148" src="https://github.com/user-attachments/assets/3ceb3037-5335-4d07-a4b5-cadae03f56d9" />

**Steps:**
1. Log in to DVWA
2. Navigate to **DVWA Security** (bottom left)
3. Select difficulty: **Low** | **Medium** | **High** | **Impossible**
4. Click **Submit**

Each vulnerability can be tested at all 4 levels independently.
